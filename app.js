...
const createGoogleDriveFolder = async (accessToken, instanceUrl, googleDriveFolderPath, sfFileId, sfContentDocumentLinkId, sfNamespace, sfCreateLog) => {
  return new Promise((resolve, reject) => {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) { 
        const createFileMigrationLogResult1 =  createFileMigrationLog(accessToken, instanceUrl, sfFileId, sfContentDocumentLinkId, 'TEST MESSGAE Onload 11', sfNamespace); 
        const response = JSON.parse(xhr.responseText);
        console.log(response);
